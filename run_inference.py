...
def main():
    ...
    #     logits_a, decoder_a, blank_id_a = extract_logits(OUTPUT_PATH_A, device)
    #     logits_b, _, _ = extract_logits(OUTPUT_PATH_B, device)

    #     logger.info("Starting posterior fusion and decoding...")
    #     fused_sequences = []
    #     utterance_ids = []
        
    #     for utt_id, seq_a in logits_a.items():
    #         seq_b = logits_b[utt_id]
    #         fused_seq = align_and_fuse_sequence(
    #             sequence_a=seq_a,
    #             sequence_b=seq_b,
    #             weight_a=WEIGHT_A,
    ...
    #     logger.info("Decoding ensemble...")
    #     decode_batch_size = 256
    #     for start_idx in range(0, len(utterance_ids), decode_batch_size):
    #         end_idx = min(len(utterance_ids), start_idx + decode_batch_size)
    #         batch_ids = utterance_ids[start_idx:end_idx]
    #         batch_sequences = fused_sequences[start_idx:end_idx]
    #         # Convert to right format and move to device if needed
    #         batch_sequences = [seq.to(torch.float16) for seq in batch_sequences]
    #         batch_predictions = decoder_a(batch_sequences)
    #         for utt_id, pred in zip(batch_ids, batch_predictions):
    #             predictions[utt_id] = pred

    # else:
    #     # Fallback to normal inference logic if ENSEMBLE = False
    #     config = OUTPUT_PATH / ".hydra" / "config.yaml"
    #     cfg = OmegaConf.load(config)
    ...
    #     cfg.preprocessing.min_duration_sec = 0 # during inference, don't filter out the too short ones
    #     cfg.training.dataloader.batch_size = 64  # A100 80GB 
    #     cfg.training.dataloader.num_workers = 4  # 24 vCPUs available in runtime
    #     cfg.model.decoder._target_ = "src.utils.decoder.BeamSearchDecoder"
    #     cfg.model.decoder.beam_width = 50
    #     # cfg.model.decoder.temperature = 1.0
    #     # cfg.model.decoder.blank_penalty = 0.25
    #     # cfg.model.decoder.alpha = 0.102
    ...
    #         state_dict = torch.load(model_path, map_location="cpu")['ema_state_dict']
    #         state_dict = {k[len("ema_model."):]: v for k, v in state_dict.items() if k.startswith("ema_model.")}

    #         model.load_state_dict(state_dict)
    #     else:        
    #         logger.info("No EMA weights found in checkpoint, loading regular model weights...")
    ...
    #     demucs_enabled = bool(ENABLE_RUNTIME_DEMUCS)
    #     demucs_model = None
    #     if demucs_enabled:
    #         model_target = str(cfg.model.get("_target_", ""))
    #         if "Wav2Vec2" not in model_target:
    #             logger.info("[Demucs] Runtime Demucs disabled: current model is not waveform-based.")
    #             demucs_enabled = False
    #         else:
    #             demucs_repo = RUNTIME_DEMUCS_REPO_DIR if RUNTIME_DEMUCS_REPO_DIR.exists() else None
    #             demucs_model = _load_runtime_demucs_model(
    #                 RUNTIME_DEMUCS_MODEL,
    #                 device=device,
    #                 repo_dir=demucs_repo,
    #             )
    #             logger.info(
    #                 "[Demucs] Runtime enabled with model='{}', repo='{}', child_original_mix={:.2f}",
    #                 RUNTIME_DEMUCS_MODEL,
    #                 str(demucs_repo) if demucs_repo is not None else "remote",
    #                 DEMUCS_CHILD_ORIGINAL_MIX,
    #             )

    #     logger.info(f"Processing {dataset_info['train_size']} utterances")
    ...
    #     original_selected = 0
    #     demucs_generated = 0
    #     demucs_mode_logged = False
    #     all_original_log_probs: dict[str, torch.Tensor] = {}
    #     all_demucs_log_probs: dict[str, torch.Tensor] = {}
    #     logger.info("Starting stage 1/2: collecting log-prob sequences...")
    #     with torch.no_grad():
    #         for batch in data_loader:
    #             with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=torch.cuda.is_available()):
    #                 input_features = batch["input_features"].to(device)
    ...
    #                 use_demucs = demucs_enabled and input_features.ndim == 2
    #                 if demucs_enabled and input_features.ndim != 2 and not demucs_mode_logged:
    #                     logger.info("[Demucs] Disabled for this run: model inputs are not raw waveforms.")
    #                     demucs_mode_logged = True

    #                 if use_demucs:
    #                     demucs_features = _build_runtime_demucs_batch(
    #                         original_audio=input_features,
    #                         input_lengths=input_lengths,
    #                         demucs_model=demucs_model,
    #                         child_original_mix=DEMUCS_CHILD_ORIGINAL_MIX,
    #                     )
    #                     demucs_generated += int(input_features.shape[0])
    #                     original_log_probs = infer_original_log_probs(
    #                         model=model,
    #                         input_features=input_features,
    #                         input_lengths=input_lengths,
    #                         attention_mask=attention_mask,
    #                     )
    #                     demucs_log_probs = infer_original_log_probs(
    #                         model=model,
    #                         input_features=demucs_features,
    #                         input_lengths=input_lengths,
    #                         attention_mask=attention_mask,
    #                     )

    #                     for utt_id, seq in zip(batch["utterance_ids"], original_log_probs):
    #                         all_original_log_probs[utt_id] = seq
    #                     for utt_id, seq in zip(batch["utterance_ids"], demucs_log_probs):
    #                         all_demucs_log_probs[utt_id] = seq
    #                 else:
    #                     original_log_probs = infer_original_log_probs(
    #                         model=model,
    #                         input_features=input_features,
    ...
    #     logger.info(
    #         "Collected log-probs: original={} demucs={}.",
    #         len(all_original_log_probs),
    #         len(all_demucs_log_probs),
    #     )

    #     logger.info("Starting stage 2/2: decoding and selecting predictions...")
    #     decode_batch_size = 256
    #     ordered_utt_ids = list(all_original_log_probs.keys())

    #     original_predictions = decode_log_probs_in_batches(
    #         decoder=model.decoder,
    #         utterance_ids=ordered_utt_ids,
    #         log_probs_by_utt=all_original_log_probs,
    #         batch_size=decode_batch_size,
    #         desc="Decoding original",
    #     )

    #     demucs_predictions: dict[str, str] = {}
    #     if all_demucs_log_probs:
    #         demucs_utt_ids = list(all_demucs_log_probs.keys())
    #         demucs_predictions = decode_log_probs_in_batches(
    #             decoder=model.decoder,
    #             utterance_ids=demucs_utt_ids,
    #             log_probs_by_utt=all_demucs_log_probs,
    #             batch_size=decode_batch_size,
    #             desc="Decoding demucs",
    #         )

    #     tokenizer = dataset_info["tokenizer"]
    #     for utt_id in ordered_utt_ids:
    #         pred_orig = original_predictions[utt_id]
    #         pred_demucs = demucs_predictions.get(utt_id)
    #         if pred_demucs is None:
    #             predictions[utt_id] = pred_orig
    #             original_selected += 1
    #             continue
    ...
    #             original_selected += 1
    #         else:
    #             predictions[utt_id] = pred_demucs
    #             demucs_selected += 1

    #     logger.info(
    #         "Demucs select: demucs={} original={} runtime_demucs_generated={}",
    #         demucs_selected,
    #         original_selected,
    #         demucs_generated,
    #     )
